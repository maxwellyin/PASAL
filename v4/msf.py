...
from utils.experiments import load_raw_target_datasets, self_train_once, zero_prompt_embedding
...
def parse_args():
    parser = argparse.ArgumentParser(description="Model self-training from a source model with a zeroed frozen prompt.")
    ...
    parser.add_argument("--source-run-name", default="12", help="Source-side checkpoint subdirectory.")
    parser.add_argument("--target-run-name", default="13", help="Target-side checkpoint subdirectory.")
...
def main():
    ...
    checkpoint_path = args.checkpoint_path or (
        args.output_root / f"prompt-{model_name}-{args.source_domain}" / args.source_run_name / "final"
    )
    output_dir = args.output_root / f"prompt-{model_name}-{args.source_domain}" / args.target_run_name / args.target_domain
