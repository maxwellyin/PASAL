...
def parse_args():
    parser = argparse.ArgumentParser(description="Alternate prompt-only and model-only self-training across multiple rounds.")
    ...
    parser.add_argument("--prompt-epochs", type=int, default=1, help="Epochs for prompt-only rounds.")
    parser.add_argument("--model-epochs", type=int, default=1, help="Epochs for model-only rounds.")
    parser.add_argument("--prompt-learning-rate", type=float, default=1e-3, help="Learning rate for prompt-only rounds.")
    parser.add_argument("--model-learning-rate", type=float, default=5.6e-5, help="Learning rate for model-only rounds.")
    ...
    parser.add_argument("--rounds", type=int, default=6, help="Number of adaptation rounds.")
    parser.add_argument("--qg-model", default="lmqg/flan-t5-base-squad-qg", help="Question-generation checkpoint.")
    parser.add_argument("--run-name", default="15", help="Subdirectory used for checkpoint naming.")
...
def main():
    ...
    output_dir = args.output_root / f"prompt-{model_name}-{args.source_domain}" / args.run_name / args.target_domain
    ...
    for round_index in range(args.rounds):
        ...
        prompt_round = round_index % 2 == 0
        strategy = "prompt_only" if prompt_round else "freeze_prompt"
        epochs = args.prompt_epochs if prompt_round else args.model_epochs
        learning_rate = args.prompt_learning_rate if prompt_round else args.model_learning_rate
        round_output_dir = output_dir / ("psf" if prompt_round else "msf")
        model, outcome, pseudo_dataset, filtered_dataset = self_train_once(
            ...
            trainable_strategy=strategy,
            use_prompt=True,
        )
        outcomes.append(outcome)
        model.save_pretrained(str(output_dir / "final" / str(round_index)))
