...
# %%
def train_prompt(train_dataset, validation_dataset, model):
    ...
    args = Seq2SeqTrainingArguments(
        output_dir=f"./checkpoint/prompt-{model_name}-{SOURCE_DOMAIN}/15/{TARGET_DOMAIN}/psf",
...
def train_model(train_dataset, validation_dataset, model):
    ...
    # Show the training loss with every epoch
    args = Seq2SeqTrainingArguments(
        output_dir=f"./checkpoint/prompt-{model_name}-{SOURCE_DOMAIN}/15/{TARGET_DOMAIN}msf/",
        evaluation_strategy="epoch",
...
def main(i:int, model):
    ...
    question_answerer = pipeline("text2text-generation", model=model, tokenizer=tokenizer, device=torch.cuda.is_available()-1)

    def create_pseudo_label(example):
        qg_input = format_for_lmqg(example)
        output_qg = question_generation(qg_input)[0]['generated_text']
        qa_input = tokenizer.pad_token*PROMPT_LENGTH + f"question: {output_qg} context: {example['context']}"
    ...
    if i % 2 == 0:
        model, outcome = train_prompt(pseudo_set2, raw_datasets['test'], model)
    else: 
        model, outcome = train_model(pseudo_set2, raw_datasets['test'], model)
    return model, outcome
# %%
outcomes = []
for i in range(6):
    ...
    model.save_pretrained(f"./checkpoint/prompt-{model_name}-{SOURCE_DOMAIN}/15/{TARGET_DOMAIN}/final/{i}")
