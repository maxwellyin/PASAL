...
from utils.runtime import DEFAULT_DATA_ROOT, V4_ROOT, model_slug, resolve_data_dir
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate a PASAL checkpoint on a target-domain test split.")
    ...
    parser.add_argument("--checkpoint-path", type=Path, default=None, help="Checkpoint directory to evaluate. Defaults to the prompt-tuned base model.")
...
def main():
    ...
    checkpoint_path = args.checkpoint_path or (V4_ROOT / "checkpoint" / f"prompt-{model_name}-{args.source_domain}" / "base" / "final")
    model = PromptT5.from_pretrained(str(checkpoint_path))

    training_args = Seq2SeqTrainingArguments(
        output_dir=str(V4_ROOT / "out"),
